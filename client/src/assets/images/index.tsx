import Store_1 from "./Store_1.jpg";
import Formal_men from "./Fomal_men.jpg";
import Wedding_attire from "./Wedding_attire.jpg";
import Tailoring from "./Tailoring.jpg";
import Hanging_clothes from "./Hanging_clothes.jpg";
import Standalone from "./Standalone.jpg";
import Wedding_252x168 from "./Wedding_252x168.jpg";
import Tailoring_252x168 from "./Tailoring_252x168.jpg";
import Coat_display from "./Coat_display.jpg";
import Dress_display from "./Dress_display.jpg";
import Product_sample from "./Product_sample.jpg";
import Product1 from "./product1.webp";
import Product2 from "./product2.webp";
import Product3 from "./product3.webp";
import Product4 from "./product4.webp";
import SizeChart from "./SizeChart.jpg";
import Tailoring_424x636 from "./Tailoring_424x636.jpg";
import Logo from "./logo.svg";
import Logo2 from "./logo2.svg";
import Package1 from "./Package1.jpg";
import Package2 from "./Package2.jpg";

export {
  Store_1,
  Formal_men,
  Wedding_attire,
  Tailoring,
  Hanging_clothes,
  Standalone,
  Wedding_252x168,
  Tailoring_252x168,
  Coat_display,
  Dress_display,
  Product_sample,
  Product1,
  Product2,
  Product3,
  Product4,
  SizeChart,
  Tailoring_424x636,
  Logo,
  Package1,
  Package2,
  Logo2,
};
